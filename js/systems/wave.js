...
import { getEnemyCap, getEnemyQualityMultiplier } from './performance.js';
...
export class WaveSystem{
  ...
  update(dt){
    ...
    const countMult=this.game.challengeModifiers?.enemyCountMultiplier||1;
    const hasBoss=this.game.enemies.some(e=>e.flags?.boss);
    const maxEnemies=getEnemyCap(this.game, hasBoss);
    const qualityMult=getEnemyQualityMultiplier(this.game, maxEnemies);
    if(this.timer>0 && this.spawnTimer<=0 && this.game.enemies.length<maxEnemies){
      ...
      const baseCount=wave>10?randInt(3,6):wave>5?randInt(2,4):wave>2?randInt(1,3):1;
      const count=Math.max(1,Math.round(baseCount*countMult));
      for(let i=0;i<count;i++)this.spawnEnemy(this.pickType(wave), null, null, qualityMult);
    }
    if(wave%CONFIG.wave.bossEvery===0 && !this.bossSpawned && this.timer<CONFIG.wave.duration*.76){ this.bossSpawned=true; this.spawnEnemy('boss', null, null, qualityMult); this.game.log('Boss出現！範囲予兆に注意'); }
  ...
  spawnEnemy(type, forcedX=null, forcedY=null, qualityMult=1){
    ...
    this.game.enemies.push(new Enemy(type,this.game.wave,x,y,this.game.depth,this.game.challengeModifiers||{}, qualityMult));
  }
}
