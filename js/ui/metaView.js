...
export function setupMeta(app){
  ...
  $('#upgradeCategoryBtn').addEventListener('click',()=>{ const cat=prompt('一括強化するカテゴリ名を入力: 基礎 / 攻撃 / 防御 / 補助 / 報酬 / 召喚 / 処刑'); if(!cat)return; for(const def of UPGRADE_DEFS.filter(u=>u.category===cat)){ while(canBuy(app.save,def)) buyOne(app.save,def); } saveGame(app.save); renderMeta(app); });
