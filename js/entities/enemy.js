...
export class Enemy{
  constructor(typeId, wave, x, y, depth={enemyHp:1, enemyAtk:1}, challengeMods={}, qualityMult=1){
    ...
    const depthTier=Math.max(0, (depth.id||1)-1);
    const quality=1 + (qualityMult-1)*0.55;
    this.maxHp=t.hp*hpScale*depth.enemyHp*(challengeMods.enemyHpMultiplier||1)*(t.boss?1+wave*0.22:1)*(t.elite?1+wave*0.035:1)*quality*(1+depthTier*0.09); this.hp=this.maxHp;
    this.speed=t.speed*(1+wave*0.014)*(challengeMods.enemySpeedMultiplier||1); this.damage=t.damage*atkScale*depth.enemyAtk*(challengeMods.enemyDamageMultiplier||1)*(1+(qualityMult-1)*0.4)*(1+depthTier*0.06); this.xp=t.xp*Math.pow(1.1,wave-1)*(1+(qualityMult-1)*0.75);
    ...
    this.critResist=Math.min(.7,(t.critResist||0)+wave*.006+(t.boss?.08:0)+depthTier*0.018);
    this.statusResist=Math.min(.82,(t.statusResist||0)+wave*.008+(t.boss?.1:0)+depthTier*0.015);
    this.areaResist=Math.min(0.65,(t.areaResist||0)+depthTier*0.02);
    this.buff=1;
  }
  update(dt, game){
  ...
  tryShoot(dt, game, d){
    ...
    for(let i=0;i<Math.max(1,Math.round(projectileMult));i++){ for(const s of spread){
      ...
      game.projectiles.push(new Projectile(this.x,this.y,null,this.damage*.9,{owner:'enemy',vx,vy,speed:this.flags.boss?300:260,r:this.flags.boss?7:5,life:this.flags.ranged?2.4:3,color:this.flags.boss?'#ff5d73':'#80d7ff'}));
    }}
