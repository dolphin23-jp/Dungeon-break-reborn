...
import { equipItem, dismantleItem } from './systems/loot.js';
...
class App{
  ...
  showInventory(){
    ...
    $('#dismantleCommonBtn').onclick=()=>{ const ids=(this.save.inventory||[]).filter(x=>(RARITIES[x.rarity]?.order||0)<=2).map(x=>x.id); ids.forEach(id=>dismantleItem(this.save,id)); this.showInventory(); this.refreshTitle(); };
  ...
  help(){ modal(`<h2>操作説明</h2><div class="help-list"><p>PC: WASD / 矢印キーで移動。Mキーまたはボタンで手動移動とオート移動を切替。</p><p>iPad/スマホ: 左下の仮想スティックで移動。攻撃とスキルは自動発動します。</p><p>Phase 3では深度選択、実績一覧、装備管理、分解、セーブエクスポート/インポート、ビルド確認を追加しました。</p><p>深度を上げるほど敵HP・攻撃・出現数が増え、深淵石と高レア装備が大きく増えます。</p></div><div class="modal-actions"><button id="closeHelpBtn" class="btn primary">閉じる</button></div>`); $('#closeHelpBtn').onclick=closeModal; }
}
