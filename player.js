...
export class Player{
  constructor(save){
    ...
    this.maxHp = CONFIG.player.baseHp + lv('vitality')*4 + sumBonus(eq,'hp');
    this.hp = this.maxHp; this.shield = Math.floor(this.maxHp*(lv('barrier')*0.01 + sumBonus(eq,'barrier')));
    this.level = 1; this.xp = 0; this.nextXp = CONFIG.xp.base;
    this.baseDamage = CONFIG.player.baseDamage + lv('power')*1.8 + lv('summon')*0.8 + sumBonus(eq,'damageFlat');
    this.baseSpeed = CONFIG.player.baseSpeed + lv('speed')*1.5;
    this.baseAttackSpeed = CONFIG.player.baseAttackSpeed * (1 + lv('haste')*0.018);
    this.baseRange = CONFIG.player.baseRange + lv('range')*2.2 + sumBonus(eq,'range');
    ...
    this.elemental = lv('elemental')*0.012 + sumBonus(eq,'elemental');
    this.mods = {
    ...
    this.attackTimer = 0; this.auraTimer = 0; this.stormTimer = 0; this.minionTimer = 0; this.barrierTimer = 0;
  }
